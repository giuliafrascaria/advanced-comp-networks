		...
		inout standard_metadata_t std_meta)
{
	...
	state parse_udp {
		...
		transition select(hdr.udp.dstPort) {
			UDP_PORT_KV: parse_pckt;//parse on specific port, 4242
			default: accept;
		}
	}
		   ...
		   inout standard_metadata_t std_meta)
{
	...
	{
		kv_regs.read(val, key);//read(destnation buffer, index)
		std_meta.egress_spec = std_meta.ingress_port;
	}

	action put_val(in key_t key, in value_t val)
	{
		kv_regs.write(key, val);//write(index, value)
		std_meta.egress_spec = std_meta.ingress_port;
	}
	...
	apply
	{
		if (hdr.pckt.type == TYPE_GET_REQ)//distinguish get from put request
		{
...
{
	apply{
        	pckt.emit(hdr.eth);//reassemble headers
		pckt.emit(hdr.ipv4);
		pckt.emit(hdr.udp);
		pckt.emit(hdr.pckt);
	}
