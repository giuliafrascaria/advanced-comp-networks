...
parser MyParser(packet_in packet,
	...
	{
		0x1234: parse_srcRouting; //source routing type
		default: accept;
	}
    }
...
control MyIngress(inout headers hdr,
    ...
    action srcRoute_nhop() {
         ...
         */
        standard_metadata.egress_spec = (bit<9>) hdr.srcRoutes[0].port; //top of the stack is the new egress port
	hdr.srcRoutes.pop_front(1); //pop the port we just used
    }

    action srcRoute_finish() {
        hdr.ethernet.etherType = TYPE_IPV4; //remove srcrouting header
    }
    ...
    apply {
        if (hdr.srcRoutes[0].isValid()){
             ...
             */
	    if (hdr.srcRoutes[0].bos == 1) //if stack is empty, just treat as plain IP
	    {
		srcRoute_finish();
	    }
